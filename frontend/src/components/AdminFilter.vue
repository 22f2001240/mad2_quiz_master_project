        ...
        <div class="container">
            <div class="filter-container">
                ...
                <div class="filter-buttons">
                    ...
                    <button :class="{ active: activeFilter === 'user' }" @click="filterBy('users')">
                        👤 Users
                    </button>
            ...
            <table v-if="searchQuery && filteredCategoryItems.length" class="table-auto border-collapse  border border-gray-600">
                <thead>
                    <tr>
                        ...
                        <template v-if="activeFilter=='user'">
                            <th class="border border-gray-300 px-4 py-2">Email ID</th>
                            <th class="border border-gray-300 px-4 py-2">DOB</th>
                            <th class="border border-gray-300 px-4 py-2">Num of participated quizzes</th>
                            <th class="border border-gray-300 px-4 py-2">Participated quizzes</th>
                        </template>
                ...
                <tbody>
                    <tr v-for="item in filteredCategoryItems" :key="item.id">
                        ...
                        <template v-if="activeFilter=='user'">
                            <td class="border border-gray-300 px-4 py-2">{{ item.email || 'N/A' }}</td>
                            <td class="border border-gray-300 px-4 py-2">{{ item.dob || 'N/A' }}</td>
                            <td class="border border-gray-300 px-4 py-2">{{ item.num_of_attempts || 0 }}</td>
                            <td class="border border-gray-300 px-4 py-2">{{ item.attempted_quizzes || 'Nill' }}</td>
                        </template>
...
export default {
        ...
        filteredCategoryItems() {
                ...
                if(this.searchQuery){
                    selectedCategoryItems = selectedCategoryItems.filter(item => 
                        ...
                        (item.description?.toLowerCase().includes(this.searchQuery.toLowerCase()) ||false)
                    );
                }
    ...
    methods : {
        async filterBy(category) {
            ...
            if (category == 'users') {
                this.selectedCategory = this.users
